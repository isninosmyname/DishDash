    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 px-4 md:px-6 mb-12 md:mb-20">
      {cards.map((card) => (
        <div key={card.title} className={`${card.bgColor} rounded-[24px] md:rounded-[40px] overflow-hidden relative group min-h-[260px] md:min-h-[420px] ${loading ? 'opacity-80 cursor-wait pointer-events-none' : 'cursor-pointer'}`} onClick={loading ? undefined : card.onClick}>
          ...
          <div className="relative p-6 md:p-10 h-full flex flex-col justify-start items-start">
            <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${card.btnColor} text-[10px] font-black uppercase tracking-widest mb-4 md:mb-6`}>
              {card.icon}
              {card.type}
            </span>
            <h2 className="text-white text-2xl md:text-5xl font-black mb-2 md:mb-4 tracking-tighter">
              {card.title}
            </h2>
